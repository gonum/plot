...
func (c *Canvas) Fill(path vg.Path) {
	c.svg.Path(c.pathData(path),
		c.style(elm("fill", "#000000", colorString(c.context().color)),
...
func (c *Canvas) WriteTo(w io.Writer) (int64, error) {
	...
	// write styles with names in ascending order
	type styleInfo struct{ name, style string }
	styles := make([]styleInfo, 0, len(c.styles))
	for style, name := range c.styles {
		styles = append(styles, styleInfo{name: name, style: style})
	}
...
// returned.
func (c *Canvas) style(elms ...string) string {
	str := ""
	for _, e := range elms {
		if e == "" {
			continue
...
func (c *Canvas) classNameOf(style string) string {
	...
	name := "st" + strconv.Itoa(len(c.styles))
	c.styles[style] = name
	return name
}
