...
func (h *Contour) Plot(c draw.Canvas, plt *plot.Plot, x, y *plot.Axis) {
	if naive {
		h.naivePlot(c, plt, x, y)
		return
	}
...
func (h *Contour) naivePlot(c draw.Canvas, plt *plot.Plot, x, y *plot.Axis) {
	var pal []color.Color
	if h.Palette != nil {
		pal = h.Palette.Colors()
	}
...
func (h *Contour) GlyphBoxes(plt *plot.Plot, x, y *plot.Axis) []plot.GlyphBox {
	c, r := h.GridXYZ.Dims()
