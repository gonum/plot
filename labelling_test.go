...
func TestTalbotLinHanrahan(t *testing.T) {
	for _, test := range talbotLinHanrahanTests {
		values, step, magnitude := talbotLinHanrahan(test.dMin, test.dMax, test.want, test.contained, nil, nil, nil)
		if !reflect.DeepEqual(values, test.wantValues) {
			t.Errorf("unexpected values for dMin=%g, dMax=%g, want=%d, contained=%t:\ngot: %v\nwant:%v",
				test.dMin, test.dMax, test.want, test.contained, values, test.wantValues)
		}
		if step != test.wantStep {
			t.Errorf("unexpected step for dMin=%g, dMax=%g, want=%d, contained=%t: got:%v want:%v",
				test.dMin, test.dMax, test.want, test.contained, step, test.wantStep)
		}
		if magnitude != test.wantMagnitude {
			t.Errorf("unexpected magnitude for dMin=%g, dMax=%g, want=%d, contained=%t: got:%d want:%d",
				test.dMin, test.dMax, test.want, test.contained, magnitude, test.wantMagnitude)
		}
		if test.contained {
			f := math.Pow10(-magnitude)
			if test.contained && (test.dMin*f < values[0]*f || values[len(values)-1]*f < test.dMax*f) {
				t.Errorf("unexpected values for contained dMin=%g, dMax=%g, want=%d not contained:\ngot: %v\nwant:%v",
